...
function Spin2WinWheel() {
  var xmlns = 'http://www.w3.org/2000/svg',
    ...
    setInitData = function () {
      ...
      pegSnd = new Audio(dataObj.spinSound)
    ...
    drawSegments = function () {
      ...
      for (var i = 0; i < numSegments; i++) {
        ...
        if (segmentValuesArray[i].mainPrize) {
          p.setAttributeNS(null, 'fill', '#fbd499')
        }
    ...
    addValues = function () {
      ...
      for (var i = 0; i < numSegments; i++) {
        ...
        var segment = segmentValuesArray[i]
        //var c = document.createElementNS(xmlns, 'circle');
        if (segment.type === 'image') {
          var segmentImageSize = segment.wheelImageSize || wheelImageSize
          var ig = document.createElementNS(xmlns, 'image')
          var useClip
          var prizeFrame

          var xCoord = centerX - (segmentImageSize / 2)
          var yCoord = centerY - wheelRadius + wheelImageOffsetY


          if (segment.mainPrize) {
            var clipID = 'circular-image-clip'
            var mainPrizeImageSize = segmentImageSize - (segment.mainPrize.reduceImageBy || 0)
            var mainPrizeXCoord = centerX - (mainPrizeImageSize / 2)

            defs.innerHTML += `
              <rect 
                id="circular-image" 
                x="${mainPrizeXCoord}" 
                y="${yCoord + (segmentImageSize * 0.25)}" 
                width="${mainPrizeImageSize}" 
                height="${mainPrizeImageSize}" 
                rx="${mainPrizeImageSize}"
              />
              <clipPath id="${clipID}">
                <use xlink:href="#circular-image"/>
              </clipPath>
            `
            useClip = createEl('<use xlink:href="#circular-image" stroke-width="2" stroke="black"/>')

            ig.setAttribute('clip-path', `url(#${clipID})`)
            g.appendChild(useClip)

            prizeFrame = document.createElementNS(xmlns, 'image')
            prizeFrame.setAttributeNS(null, 'x', xCoord)
            prizeFrame.setAttributeNS(null, 'y', yCoord)
            prizeFrame.setAttributeNS(null, 'width', segmentImageSize)
            prizeFrame.setAttributeNS(null, 'height', segmentImageSize)
            prizeFrame.setAttributeNS(xlinkns, 'xlink:href', segment.mainPrize.frameImage)

            if (!segment.mainPrize.isFrameOverlay) g.appendChild(prizeFrame)

            ig.setAttribute('class', 'wheelImage')
            ig.setAttributeNS(null, 'x', mainPrizeXCoord)
            ig.setAttributeNS(null, 'y', yCoord + (segmentImageSize * 0.25))
            ig.setAttributeNS(null, 'width', mainPrizeImageSize)
            ig.setAttributeNS(null, 'height', mainPrizeImageSize)
            ig.setAttributeNS(xlinkns, 'xlink:href', segment.value)
            ...
            ig.setAttribute('class', 'wheelImage')
            ig.setAttributeNS(null, 'x', xCoord)
            ig.setAttributeNS(null, 'y', yCoord)
            ig.setAttributeNS(null, 'width', segmentImageSize)
            ig.setAttributeNS(null, 'height', segmentImageSize)
            ig.setAttributeNS(xlinkns, 'xlink:href', segment.value)
          ...
          if (segment.text) {
            var imageText = document.createElementNS(xmlns, 'text')
            var imageLines = segment.text.split('^')
            var imageTextOffset = segmentImageSize + (segment.wheelTextOffsetY || wheelTextOffsetY || 0) + (segment.wheelImageOffsetY || wheelImageOffsetY)

            imageLines.forEach(function (value, index) {
              ...
              ts.appendChild(tn)
              imageText.appendChild(ts)
            })

            imageText.setAttribute('class', 'wheelText')
            imageText.setAttributeNS(null, 'fill', wheelTextColor)
            imageText.setAttributeNS(null, 'x', centerX)
            // imageText.setAttributeNS(null, 'y', centerY - wheelRadius + imageTextOffset)
            imageText.setAttributeNS(null, 'y', centerY - wheelRadius + imageTextOffset)
            imageText.style.fontSize = wheelTextSize

            g.appendChild(imageText)
          }
        } else if (segment.type === 'string') {

          var t = document.createElementNS(xmlns, 'text')
          var lines = segment.value.split('^')
          var textOffset = segment.wheelTextOffsetY || wheelTextOffsetY
          ...
          t.setAttributeNS(null, 'y', centerY - wheelRadius + textOffset)
  ...
  var createProbabilityArray = function () {
    ...
    getTrigger = function (wheelClass) {
      return function (segmentToWin) {
        if (hasProbability) {
          ...
          if (segmentToWin) {
            spinDestinationArray = [segmentToWin]
            setSpinDestinations()
          }
